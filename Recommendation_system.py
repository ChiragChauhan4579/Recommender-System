...
query_index = input('Enter the no. of your movie: ')
print(query_index)
for i in range(0,len(movie_rating_count_)):
    if movie_rating_count_.iloc[i]['title']== query_index:
         movie_id = i
#print(a)
distances,indices = model_knn.kneighbors(movie_features.iloc[movie_id,:].values.reshape(1,-1),n_neighbors=6)
print(movie_features.iloc[movie_id,:])

for i in range(0,len(distances.flatten())):
    if i == 0:
        print('Recommendation for {0}'.format(movie_features.index[movie_id]))
    else:
        print('{0}:{1}'.format(i,movie_features.index[indices.flatten()[i]],distances.flatten()[i],movie_features.index[movie_id]))